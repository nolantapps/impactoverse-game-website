...
const UnityWebGLTurtleGameComponent = () => {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("isChild") === "true") {
      setCheckBoxPopUp(false)
    }
  ...
  const handleAuthentication = async () => {
    try {
      ...
      sessionStorage.setItem('primary', mainAccessCode);
      sessionStorage.setItem('secondary', secondaryAccessCode)
  ...
  return (
    ...
    >
      <ScreenTimeTracker gameId="68096fc015547a91e1c140a2" />
      <iframe
        title="Turtle Guardians"
        src={`/Turtle Guardians/index.html?userID=${DataStore.getUserId()}&userName=${DataStore.getUsername()}`}
        style={{
          width: '90vw',
          height: '100vh',
          border: 'none',
...
export default UnityWebGLTurtleGameComponent;
