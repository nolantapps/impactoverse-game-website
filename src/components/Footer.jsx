...
export default function Footer() {
  ...
  const facebookLink = "https://www.facebook.com/share/16NJ9naU7k/?mibextid=wwXIfr";
  const linkedinLink = "https://www.linkedin.com/company/impactoverse/"
  const instagramLink = "https://www.instagram.com/impactoverse?igsh=ajJmczJyMzQ4NTVw&utm_source=qr"


  const footerClasses = isDarkMode 
    ? "bg-[#0d102d]" 
    : "bg-white";  // Light mode background set to white
        ...
        <div className="flex flex-col items-center space-y-8 md:space-y-4 
          ...
          <div className="flex flex-col items-center space-y-6 
            ...
            <div className="flex flex-wrap justify-center gap-6">
              {/* Instagram */}
              <a href={instagramLink} target="_blank" rel="noopener noreferrer" className={`${socialIconClasses} transition-colors`}>
                <FaInstagram  className={`w-6 h-6 ${isDarkMode ? '' : 'filter brightness-50'}`}/>
              </a>
              {/* Facebook */}
              <a href={facebookLink} target="_blank" rel="noopener noreferrer"  className={`${socialIconClasses} transition-colors`}>
                
                <FaFacebook className={`w-6 h-6 ${isDarkMode ? '' : 'filter brightness-50'}`}/>
              </a>
              {/* Linkedin */}
              <a href={linkedinLink} target="_blank" rel="noopener noreferrer" className={`${socialIconClasses} transition-colors`}>
                <FaLinkedin className={`w-6 h-6 ${isDarkMode ? '' : 'filter brightness-50'}`}/>
              </a>
            </div>
          </div>
